...
class SurrogateModel(Base):
    ...
    def select(self, x='x_pos', y='y_pos', signal=None, N_limit=None):
        ...
        if '{} error'.format(self.z_name) in self.data['signals'].keys():
            self.z_errors = self.data['signals']['{} error'.format(self.z_name)]
    ...
    def interpolate_gpcam(self, hps_guess=None, gp_method='global', pre_optimize=False, fill='pixelwise', gpcam_PATH=None, error_relative=None, renormalize_signals=True, **kwargs):
