...
class Base():
    #TODO: Use SciAnalysis Base.py instead
...
class SurrogateModel(Base):
    ...
    def remove_duplicate_points(self, params, signals, tolerance=1e-12):
        
        self.msg('Considering duplicates in params ({} entries)'.format(len(params)), 3, 1)
        
        new_params, new_signals = [], []
        for i, (param, signal) in enumerate(zip(params, signals)):
            
            duplicate = False
            for new_param in new_params:
                d = np.sum(np.abs(param-new_param))
                if d<tolerance:
                    # The position given by 'param' already exists in 'new_params'
                    self.msg('entry {:d} ({}) already exists ({})'.format(i, repr(param), repr(new_param)), 6, 3)
                    print()
                    duplicate = True
                    break
        ...
        params, signals = np.asarray(new_params), np.asarray(new_signals)
        
        self.msg('Returning cleaned params (now {} entries)'.format(len(params)), 3, 1)
    ...
    def interpolate_gpcam(self, hps_guess=None, gp_method='global', pre_optimize=False, fill='pixelwise', gpcam_PATH=None, error_relative=None, renormalize_signals=True, hps_lock=None, convex_clip=False, **kwargs):
        ...
        # Duplicate points may cause GP training to fail
        #params, signals = self.remove_duplicate_points(params, signals)
