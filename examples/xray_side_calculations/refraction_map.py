...
class DataLines_current(DataLines):
    ...
    def _plot_extra(self, **plot_args):
        ...
        #cmap = mpl.cm.jet
        cmap = cmaps.viridis
        self.ax.pcolormesh( self.x_axis, self.y_axis, self.M, cmap=cmap, shading=shading, zorder=+50 )
    ...
    def _plot_extra_top(self, **plot_args):
        ...
        l, = self.ax_top.plot(line.x, line.y, label=line.name, **p_args)
        #l.set_dashes([5,5])
        ...
        s = '$\Delta_{{ q_z,\mathrm{{Tc}} }} (Q_z = {:.2f} \, \mathrm{{\AA}}^{{-1}}) \, (\mathrm{{\AA}}^{{-1}})$'.format(self.Qz_consider)
        self.ax_top.set_ylabel(s, size=12)
