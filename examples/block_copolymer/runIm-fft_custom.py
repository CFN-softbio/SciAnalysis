...
protocols = [ 
                ...
                #Protocols.grain_size_hex(threshold=210, invert=False, **run_args),
                #Protocols.grain_size(**run_args),
                #Protocols.thumbnails(resize=0.5, crop=0.5),
                thumb_view(name='thumb_view{:d}'.format(size_nm), size_nm=size_nm),
                ]
...
if True:
    # Single directory
    data_dir = './C48/C48 on Dow 20/'
    run_args['offset'] = [130,50]
    pattern = '*q06.tif'
    ...
    for cur_root, subdirs, files in os.walk(root_dir):
        ...
        if '/analysis/' in cur_root:
            ...
            infiles = glob.glob(cur_root + '/*.tif')
