...
""".format(fontsize, yc, xc, self.txt_id, self.txt_id+1, yc, xc, text)
    ...
    def pattern_multi_series(self, infiles, parse_re, match_names=None, source_dir='./', col_spacing=200, row_spacing=None, x0=0, y0=0, parse_row_idx=0, parse_col_idx=1, overlay_text_color='000000', verbosity=3):
        ...
        row_strs = sorted(row_strs)
        row_strs = sorted(row_strs, key=float)
        col_strs = sorted(col_strs)
    ...
    def pattern_xy_positions(self, infiles, parse_re, match_names=None, source_dir='./', x_scaling=400, y_scaling=None, im_width=400, x0=0, y0=0, parse_x_idx=0, parse_y_idx=1, overlay_text_color='000000', x_offset=0, y_offset=0, verbosity=3):
        ...
        # Figure out the image positions
        filenames = []
        xpos = []
        ypos = []
        for infile in infiles:
            ...
            if self.match(filename, match_names=match_names) and m:
                ...
                filenames.append(filename)
                xpos.append( float(m.groups()[parse_x_idx]) )
                ypos.append( float(m.groups()[parse_y_idx]) )
        ...
        for filename, xf, yf in zip(filenames, xpos, ypos):
            

            if filename is None:
                if verbosity>=1:
                    print("  ERROR: filename is None for xpos='{}' ypos='{}'".format(x, y))
            
            else:
                
                
                # Convert from index to actual x position
                #xf *= step_size
                ...
                self.add_image(source_dir+filename, x, y, width=im_width)
                
                s = filename
                h = self._height
                #self.add_text_c(s, x, y-h*0.5*0.9, color=overlay_text_color, fontsize=14)
...
if True:
    ...
    if True:
        ...
        #sample_names = 'Coffee_NP12nm'
        
        im_width = 200
        step_size = 0.05
        
        sample_names, x_scaling, y_scaling, xo, yo = 'run4_CB-aPP_1kDa_', im_width/step_size, im_width/step_size, 0, 0
        
        protocol, ext = 'thumbnails_special', 'png'
        #protocol, ext = 'circular_average_q2I', 'png'
        
        match_names = None
        #match_names = ['sample1', 'sample2'] # Turn this on to select specific samples to match
        
        source_dir = '../../analysis/{}'.format(protocol)
        pattern = '{}*_saxs.*'.format(sample_names)
        #pattern = '{}*_saxs_q2I.*'.format(sample_names) 
        
        
        parse_re = '^.+_x(-?\d+\.\d+)_y(-?\d+\.\d+)_.+\d+_saxs\.{}$'.format(ext) # In this RE, the bracketed regions denote the parts that will be used for row and column sorting.

        
        col_spacing, row_spacing = 400, 400
        ...
        layout.pattern_xy_positions(infiles, parse_re, match_names=match_names, source_dir=source_dir, x_scaling=x_scaling, y_scaling=y_scaling, x_offset=xo, y_offset=yo, im_width=im_width, verbosity=2, parse_x_idx=0, parse_y_idx=1 )
    ...
    if False:
        ...
        source_dir = '../../analysis/{}'.format(protocol)
        pattern = '{}*_saxs_q2I.*'.format(sample_names) 
        parse_re = '^.+_x(-?\d+\.\d+)_yy(-?\d+\.\d+)_.+_saxs_q2I\.{}$'.format(ext) # In this RE, the bracketed regions denote the parts that will be used for row and column sorting.
