...
########################################
verbosity = 3
pattern = 'AJ_exp1_x' # Files to consider
...
class Data2D_current(Data2D):
    def _plot_extra(self, **plot_args):
        ...
        pos = [ self.X.flatten(), self.Y.flatten() ]
        U = np.sin(np.radians(self.data))
        V = np.cos(np.radians(self.data))
        self.ax.quiver(self.X, self.Y, U, V, scale=70, pivot='mid', width=0.001, headwidth=0, zorder=20)
...
def plot_grids(results, N_list, grid=[None, None, None, None], n_grid=200, plot2d=True, plot3d=False, cmap='viridis', x_label='$x \, (\mathrm{mm})$', y_label='$y \, (\mathrm{mm})$', z_label='$z$', interpolate_cyclic=None, verbosity=3):
    ...
    for N in N_list:
        ...
        outfile = os.path.join(output_dir, signal, '{}-{}quiver-N{:04d}.png'.format(pattern, signal, N))
        plot_results(x_vals, y_vals, z_vals, outfile=outfile, plot2d=plot2d, plot3d=plot3d, grid=grid, dgrid=dgrid, cmap=cmap, alpha=0.8, x_label=x_label, y_label=y_label, z_label=z_label, interpolate_cyclic=interpolate_cyclic, verbosity=verbosity)
...
for signal in signals:
    ...
    # Single result
    if True:
        plot_grids(results, [len(z_vals)], grid=grid, n_grid=80, plot2d=True, plot3d=False, cmap=cmap, x_label=x_label, y_label=y_label, z_label=z_label, verbosity=verbosity, interpolate_cyclic=180)
