...
""".format(fontsize, yc, xc, self.txt_id, self.txt_id+1, yc, xc, text)
    ...
    def pattern_image_grid(self, file_re, source_dir='./', col_spacing=200, row_spacing=None, x0=0, y0=0, verbosity=3):
        ...
        for ic, col_str in enumerate( col_strs ):
            for ir, row_str in enumerate( row_strs ):
                ...
                if verbosity>=5:
                    print("  C000000hecking col='{}' row='{}'".format(col_str, row_str))
    ...
    def pattern_multi_series(self, infiles, parse_re, source_dir='./', col_spacing=200, row_spacing=None, x0=0, y0=0, parse_row_idx=0, parse_col_idx=1, overlay_text_color='000000', verbosity=3):
        ...
        for ir, row_str in enumerate( row_strs ):
            ...
            self.add_vtext(row_str, x, y, fontsize=50)
...
if False:
    source_dir = '../../analysis/thumbnails'
    pattern = 'C67*_th0.110_*5.00s_*_saxs.jpg'
    parse_re = '^(.+)_th\d\.\d+_(\d+\.\d)s_T\d+\.\d\d\dC_.+$'
    
    
    overlay_text_color = 'FFFFFF'
    col_spacing, row_spacing = 487, 619+15 
    outfile = 'run04-C67-thumbnails.svg'
...
if True:
    ...
    pattern = 'C67*_th0.110_*5.00s_*_saxs-fit.png'
    parse_re = '^(.+)_th\d\.\d+_(\d+\.\d)s_T\d+\.\d\d\dC_.+$'

    overlay_text_color = '000000'
    col_spacing, row_spacing = 1000, 700 
    outfile = 'run04-C67-qrpeak.svg'
...
layout.pattern_multi_series(infiles, parse_re, source_dir=source_dir, col_spacing=col_spacing, row_spacing=row_spacing, overlay_text_color=overlay_text_color)
layout.save(outfile)
